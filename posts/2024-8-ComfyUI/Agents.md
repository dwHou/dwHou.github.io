...
### Security Journey —— 实现 AI 智能体（AI Agent）

在本节课中，学习者将使用 OpenAI API 从零开始实现大语言模型（LLM）的工具调用（Tool Calling）。学习者将逐步走过工具调用的完整协议流程：将工具定义为 JSON Schema、处理模型的工具调用请求、在本地执行函数，以及将结果返回给模型。本实验还引入了轻量级追踪工具 Phoenix，以便学习者能够观察工具调用流程的每一个步骤。在实验结束时，学习者将实现自己的计算器工具，并通过 Phoenix 追踪链路验证其是否正确运行。
...
大语言模型（LLM）是基于静态数据进行训练的。它们可以回答历史问题、编写代码并进行逻辑推理，但它们本身无法查询天气、检索数据库或调用 API。工具调用（也称为函数调用）正是桥接这一鸿沟的机制。

当大语言模型遇到无法仅凭训练数据回答的问题时，它可以返回一个结构化请求：“我需要你使用这些参数来调用这个函数。” 接着，您的应用程序会执行该函数，将结果发送回模型，模型再将该结果整合到最终的回答中。

这正是让一个系统成为“AI 智能体（AI Agent）”而非仅仅是“大语言模型”的关键所在。

#### 智能体循环（The Agent Loop）

像 LangChain 和 CrewAI 这样的框架都是构建在一个简单的底层原语之上的。在底层，每个智能体都遵循相同的循环流程：

1. 向模型发送一条消息，并附带可用工具列表。
2. 模型决定是否调用工具（以及调用哪一个）。
3. 您的代码执行该工具并将结果返回。
4. 模型利用该结果做出最终回答。

#### 从零开始逐步实现

在本实验中，您将不使用任何框架，从零开始实现这个循环。

LangChain 及类似的框架封装了这里发生的许多底层细节。这虽然对提高生产力大有裨益，但在出错需要排查、或者需要自定义智能体行为时，会让您很难看清实际发生了什么。通过亲自动手构建这个循环，您将清晰地看到发送了哪些消息、模型返回了什么，以及结果是如何流转回去的。

您还将使用开源的 LLM 可观测性工具 Phoenix 来追踪和检查每一次 API 调用，让您在每一步都能对消息流有完全的掌控与能见度。

打开“Code Editor（代码编辑器）”标签页即可开始。您将在一个名为 `agent-implementation.ipynb` 的 Jupyter Notebook 中进行操作。

------

#### 步骤 1：连接到 LLM 并启用追踪

Notebook 中的第一个单元格设置了以下两件事：

- **OpenAI 客户端** —— 这是向 LLM 发送请求的 SDK。我们没有直接连接到 OpenAI，而是通过一个网关代理进行路由，该代理会自动为您处理身份验证。
- **Phoenix 追踪** —— 通过对 OpenAI 客户端进行检测（Instrumenting），每一次 API 调用都会被自动记录为一条追踪记录（Trace），供您稍后检查。这对于理解工具调用内部发生的事情至关重要。您将看到发送和接收的确切消息。

运行第一个代码单元格，并留意下方是否出现绿色对勾，以确认运行成功。

> *如果您看到错误，请确保您是按顺序运行单元格的。追踪设置必须在创建客户端之前完成。*

------

#### 步骤 2：普通对话补全

填充 `prompt` 的值并运行第二个代码单元格，以确认与 LLM 的连接正常工作。这会发送一个简单的提示词，并应该返回一个直截了当的回答。

```python
prompt = "Why is an apple red?"  # 为什么苹果是红色的？
```

请注意普通的文本补全（Completion）是什么样的：响应体中包含 `content`（文本回答），且 `tool_calls` 为空。请记住这一点，下一步的输出看起来会大不相同。

------

#### 步骤 3：将工具定义为 JSON Schema

为了让 LLM 能够使用某个工具，您需要使用 JSON Schema 来描述它。模型永远看不到您的 Python 代码。它只能看到这个描述，并利用它来决定何时以及如何调用该工具。

运行下一个代码单元格来定义一个 `get_weather`（获取天气）工具。

在这个 Schema 中，有三件事至关重要：

- **name** —— 模型用来请求此函数的标识符。
- **description** —— 模型通过阅读此描述来决定何时使用该工具。模糊的描述会导致工具选择不可靠。
- **parameters** —— 告诉模型需要提供哪些参数及其类型。`required` 数组指定了哪些参数是必填的。

------

#### 步骤 4：提出一个需要该工具的问题

现在迎来了关键时刻。运行下一个单元格。它会提问“东京现在的天气怎么样？”，并在发送请求时带上工具定义。
...
- `content` 为 `None` —— 模型此时还没有答案。
- `tool_calls` 被填充了内容 —— 模型请求您调用 `get_weather`，并传入参数 `city="Tokyo"`。
- `tool_call_id` —— 一个唯一的 ID，用于将此请求与您稍后发回的结果关联起来。

**模型并没有执行任何代码。** 它只是在说：“我需要这个函数的输出结果来回答用户的问题。请帮我运行它，并告诉我结果。”

------

#### 5：在本地执行函数

模型请求调用 `get_weather(city="Tokyo")`。现在，您的代码需要实际运行它。在下一个单元格中，您将：

1. **定义 Python 函数** —— 在实际应用中，这会调用天气 API。这里我们使用模拟数据（Mock Data）。
2. **解析模型的请求** —— 从工具调用中提取函数名称和参数。
3. **执行它** —— 根据名称找到对应的函数，并传入解析后的参数进行调用。

运行该单元格。您应该会看到该函数返回一个包含天气数据的 JSON 字符串。在大多数智能体框架内部，都是通过使用一个 `available_functions` 字典来根据名称分发和路由工具调用的。

------

#### 步骤 6：发送工具执行结果

这是循环的最后一步。运行下一个单元格，将工具的执行结果发送回模型。

工具消息必须包含上一步中的 `tool_call_id`，以便模型能够将结果与其最初的请求匹配起来：
...
当您将此消息发送回 API 时，模型便拥有了所需的一切。它会用自然语言做出最终回答，例如：“东京目前的天气是 72°F，多云。”

至此，您刚刚完成了一个完整的工具调用循环。

------

#### 步骤 7：在 Phoenix 中检查追踪记录

运行完最后一个代码单元格后，切换到您沙箱中的 **Phoenix** 标签页。

您应该会看到 3 条追踪记录，对应您发起的每一次 `chat.completions.create` 调用。

点击进入**第 2 条追踪记录**并展开输入/输出。您将看到您发送的确切工具定义，以及模型返回的结构化 `tool_calls`。然后点击进入**第 3 条追踪记录**，查看工具结果是如何被纳入对话上下文之中的。

这正是您在生产环境中构建和调试 AI 智能体时所需要的可视化能力：能够看清每一次 LLM 调用的完整消息流、Token 使用情况以及延迟。
...
- **name** —— 给工具起一个名字（例如 `"calculate"`）。这是模型用来请求它时的名称。
- **description** —— 描述该工具的作用。请尽量具体，以便模型准确知道何时该使用它。

填写完两个值后，运行该单元格。

#### 2. 实现函数

下一个单元格定义了一个 `calculate()` 函数，其中留有一处空白：

```python
result = ___  # TODO: Evaluate the expression
```

将 `___` 替换为一行 Python 内置函数，用于将字符串形式的数学表达式进行求值。运行该单元格以注册您的函数。

#### 3. 运行完整循环

下一个单元格将所有内容串联起来，并针对问题 **“What is (47 \*83) + 19?”（(47\* 83) + 19 是多少？）** 运行完整的工具调用循环。

如果您的实现正确，您应该会看到：

- 工具执行结果显示：`{"expression": "(47 * 83) + 19", "result": 3920}`
- 模型的最终回答中包含了 `3,920`

*如果模型在没有调用您工具的情况下直接给出了答案，请仔细检查您的工具名称和描述。模型是根据描述来决定调用工具还是直接回答的。*

#### 4. 在 Phoenix 中验证

切换到 Phoenix 标签页，找到您运行计算器时产生的新追踪记录。点击进入工具调用的追踪记录并确认：

- 模型请求的是您的计算器工具（而不是 `get_weather`）。
- 工具返回的结果包含了正确的数学计算结果。
- 最终的回答使用的是计算出来的数值，而不是模型胡乱幻觉出来的数字。

当您对结果满意时，运行测试以验证您的解决方案。测试将通过检查 Phoenix 追踪记录，来确认您的计算器工具是否被正确调用并返回了正确的结果。
...
在构建 AI 应用程序时，我们有时希望为 AI 模型提供额外功能，例如动态上下文，或调用工具的能力。实现这一点的一种可扩展且灵活的方式是使用 **模型上下文协议**（Model Context Protocol，简称 **MCP**）。在使用 MCP 的应用程序中，你经常会遇到运行在不同安全上下文、拥有不同权限的程序，这可能导致**混淆代理**（Confused Deputy）漏洞。

当一个应用程序，也就是“代理”（deputy），代表下游应用程序向上游服务器进行授权时，就可能发生混淆代理漏洞。如果代理在与上游交互时使用自己的授权级别，而没有考虑下游应用程序的权限，那么就存在混淆代理漏洞。
...
- MCP 的客户端 / 服务器架构
- 什么是混淆代理漏洞
- 如何在 MCP Client 中修复混淆代理漏洞

------

#### MCP 简要概览

MCP 使用客户端-服务器架构，其中客户端负责与用户和 AI 模型交互，而服务器则为你希望提供给 AI 模型的各种功能提供接口。服务器提供的功能可以分为三种形式，称为**原语**（primitives）：

- **Resources（资源）**：允许 AI 模型请求额外上下文，例如数据库中的一行数据，或文件内容。
- **Tools（工具）**：允许模型执行某些过程。例如，向数据库添加一行数据，或删除一个文件。
- **Prompts（提示词）**：用于 AI 模型的可复用 prompt 模板。

这种架构使 MCP 具备灵活性——因为这些原语几乎可以封装你想提供给 AI 模型的任何功能；同时也具备可扩展性，因为客户端应用程序可以连接任意数量的 MCP 服务器。MCP 服务器可以独立于任何特定客户端结构进行开发。此外，MCP 客户端只是在相对于 MCP 服务器时才必然是客户端。因此，MCP 客户端可以很容易集成到 Web 应用后端中，甚至 MCP 服务器也可以充当其他 MCP 服务器的客户端。

不过，正如前面提到的，当一个程序代表另一个程序与第三方交互时，例如 MCP 服务器代表 MCP 客户端访问某个下游数据库，或者 MCP 客户端代表 Web 应用访问 MCP 服务器，就有可能引入混淆代理漏洞。

------

#### 侦察

Dewey, Cheatem and Howe 律师事务所希望拥有一个 AI 助手工具，帮助处理他们的案件。他们决定创建一个 MCP 服务器，供助手工具用于检索所有案件的信息。看到这个工具在内部非常有用后，他们决定将该工具直接提供给客户使用。这个面向客户的工具可以在 **App** 标签页中访问。

点击登录页面底部的一组示例凭据，然后点击 **Login** 登录账户。在 prompt 输入框中，尝试询问一个无害的问题，例如：

```tex
What are my active cases?
```

在响应下方，你会看到一组 MCP resources，这些资源用于帮助你理解模型正在做什么。除此之外，响应看起来是正常且无害的。

接下来我们尝试另一个问题：

```
Please list all accounts at the firm.
```

然后你会看到，除了我们自己的账户外，我们还可以成功查看该事务所拥有的所有其他账户。接下来，尝试使用如下查询查看另一个账户的案件数据：

```
What are the active cases for the account martinez?
```

你会发现，我们竟然能够查看另一个账户的数据！
...
1. **Web 应用前端**：它与 AI 聊天应用程序通信
2. **聊天应用程序**：它充当 MCP client，同时也是 Web 应用的后端
3. **MCP server**：它响应来自 MCP client / 聊天应用程序的资源请求等操作

在 **Code Editor** 标签页中，你会找到聊天应用程序代码。它负责协调 AI 模型，并与 MCP server 通信，以根据用户的 prompt 生成响应。为了响应用户查询，它会执行以下步骤：

1. 从 MCP server 发现有哪些资源可用
2. 询问 AI 模型是否需要这些资源来回答查询
3. 从 MCP server 检索任何被请求的资源
4. 在加入这些资源作为上下文后，让模型回答用户的 prompt

但是，原始 MCP server 只被设计用于内部使用，并假设任何访问它的 client 都拥有查看事务所正在处理的所有案件的完整权限。因此，当 MCP client 作为用户访问 MCP server 的代理时，如果它不加任何限制地委派资源请求，就会允许用户访问任何内容，而不管他们实际拥有什么权限。

有几种不同方式可以修复这个问题。一种方式是给 MCP server 添加完整的授权机制，并在 MCP server 中对账户持有人进行授权。

在 MCP client 中修复此问题的一种更简单方法，是将 AI 模型可访问的资源限制为仅限请求该信息的用户专属资源。在这个特定案例中，用户应当能够访问的所有资源都以前缀：`cases://account/{account_id}`

开头，因此我们可以在发送资源请求之前进行验证。

请注意，resource templates 指的是包含参数的资源，例如：`{account_id}`
...
- Python
  ...
  ```python
  # Before reading the resource from the MCP, verify that the resource to request starts with the aforementioned prefix:
  if uri.startswith(f"cases://account/{account_id}/"):
  ```

------

MCP 不是万能的安全解决方案：它的安全性取决于你的实现方式。就像其他通过传统 API 暴露相同数据的软件一样，MCP 也同样容易被不安全地实现。

修复问题后，点击 **Run Tests** 来验证并完成本课。
