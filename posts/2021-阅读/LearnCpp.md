...
在第 5.1 节 -- 运算符优先级和结合性中，我们讨论了如何根据运算符的优先级和结合性对表达式求值。

我们不由会想到 <font color="brown">当二元运算符的操作数是不同类型时会发生什么？</font>

比如，`??? y { 2 + 3.5 };`

在 C++ 中，某些运算符要求它们的操作数具有相同的类型。 如果使用不同类型的操作数调用这样一个运算符，则某个或所有操作数将使用一组称为**通常算术转换（usual arithmetic conversions）**的规则隐式转换为匹配类型。
...
- 二元关系运算符：<、>、<=、>=、==、!=
- 二元按位算术运算符：&、^、|
- 条件运算符 ?:（不包括条件，它应该是 bool 类型）

##### 通常算术转换规则

非常简单，编译器有一个类型的优先列表，看起来像这样：
...
1. 如果至少一个操作数的类型在优先级列表中，则将具有较低优先级的操作数转换为具有较高优先级的操作数的类型。
1. 否则（两个操作数的类型都不在列表中），两个操作数都进行数字提升（见 8.2 -- 浮点和整数提升）。

可以使用 typeid 运算符（包含在 <typeinfo> 标头中）来查看结果类型：
...
> 操作数混合有符号和无符号时，要特别注意算术转换的发生。

#### 10.6 显式类型转换（cast）和 static_cast

8.2节我们提到，值从一种数据类型提升为更宽的数据类型时，使用隐式类型转换是ok的。由于不受控制，许多时候只有隐式类型转换并不如意，如下示例：

```cpp
int x { 10 };
int y { 4 };
double d = x / y; // does integer division, initializes d with value 2.0 instead of 2.5. Most likely, this isn’t what was intended. 
```

幸运的是，C++ 附带了许多不同的类型转换运算符 type casting operators（通常称为强制转换 <font color="brown">cast</font>），程序员可以使用它们来请求编译器执行类型转换。 因为强制转换是程序员的显式请求，所以这种形式的类型转换通常称为<font color="brown">显式类型转换</font>。
...
- 命名转换：
  ...
  4. 动态强制转换（dynamic casts），25.10节介绍
...
double d { (double)x / y }; // 写法1
double d { double(x) / y }; // 写法2
// 可以看到C-style casts语法很简单，这导致它难以识别（使代码更难阅读），甚至更难以搜索。
```

C 风格强制转换实际上是根据上下文执行各种不同的转换。 这可以包括static casts、const casts或reinterpret casts。 因此，C 风格的强制转换存在被无意误用且无法产生预期行为的风险，而使用 C++ 强制转换可以轻松避免这种情况。

##### 静态强制转换

C++ 引入了称为 <font color="blue">static_cast</font>的显示转换运算符。
...
static_cast 的主要优点是它提供编译时类型检查，从而避免犯无意的错误。

最佳实践：类型转换时倾向使用static_cast

另外，使用static_cast可以进行显式的数字变窄，而不引起warnings。

#### 10.7 Typedef 和类型别名

##### 类型别名

在 C++ 中，using 是一个关键字，可为现有数据类型创建别名。 例如：

```cpp
using Distance = double; // define Distance as an alias for type double
```

当编译器遇到类型别名时，它将替换为该现有类型。例如：
...
> 对于高级读者：类型别名也可以模板化。 我们在第 13.12 课——类模板参数推导 (CTAD) 和推导指南中介绍了这一点。
